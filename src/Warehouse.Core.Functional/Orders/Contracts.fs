...
type IMarketOrderProvider =
    ...
    abstract member ExecuteOrderAsync: order: Order * cancellationToken: CancellationToken -> Task<Result<string>>

type IOrderManager =
    abstract member CreateOrderAsync: request: CreateOrderRequest * cancellationToken: CancellationToken -> Task<Result<Order>>
    abstract member ExecuteOrderAsync: orderId: int64 * cancellationToken: CancellationToken -> Task<Result<Order>>

    abstract member UpdateOrderAsync:
        orderId: int64 * request: UpdateOrderRequest * cancellationToken: CancellationToken -> Task<Result<Order>>

    abstract member CancelOrderAsync: orderId: int64 * reason: string option * cancellationToken: CancellationToken -> Task<Result<Order>>
    ...
    abstract member GetOrdersAsync:
        workerId: int * status: OrderStatus option * cancellationToken: CancellationToken -> Task<System.Collections.Generic.List<Order>>
    ...
    abstract member GetOrderHistoryAsync:
        skip: int * take: int * filter: OrderHistoryFilter option * cancellationToken: CancellationToken ->
            Task<System.Collections.Generic.List<Order>>
