    ...
    fun ctx ->
        task {
            try
                ...
                let! total = DashboardQueries.create(scopeFactory).TotalBalanceUsdt()
                return total |> fun totalBalance -> Response.ofPlainText (string totalBalance) ctx
            with ex ->
    ...
    fun ctx ->
        task {
            try
                ...
                let! totalBalance = BalanceManager.getTotalUsdtValue balanceManager marketType CancellationToken.None
