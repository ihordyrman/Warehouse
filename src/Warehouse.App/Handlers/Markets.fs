module Warehouse.App.Handlers.Markets
...
let private marketCard (market: MarketInfo) =
    let typeClass =
        if market.Type.ToString() = "OKX" then "bg-blue-100 text-blue-800" else "bg-gray-100 text-gray-800"
    ...
    _div [
        ...
        _div [ _class_ "p-4 border-b border-gray-100 bg-gradient-to-r from-gray-50 to-white" ] [
            _div [ _class_ "flex justify-between items-start mb-3" ] [
                _div [ _class_ "flex-1" ] [
                    _h3 [ _class_ "text-lg font-bold text-gray-900 mb-1" ] [ Text.raw market.Name ]
                    _span [
                        _class_ $"inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {typeClass}"
                    ] [ _i [ _class_ "fas fa-exchange-alt mr-1" ] []; Text.raw (market.Type.ToString()) ]
                ]
                _span [ _class_ $"inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold {statusClass}" ] [
        ...
        _div [ _class_ "p-4 space-y-3" ] [
            ...
            _div [ _class_ "pt-4 border-t border-gray-100" ] [
                ...
                _div [
                    ...
                    _div [ _class_ "flex justify-center py-2" ] [
                        _i [ _class_ "fas fa-spinner fa-spin text-gray-400" ] []
                    ]
                ]
            ]
        ]
        _div [ _class_ "px-4 py-3 bg-gray-50 border-t border-gray-100 flex justify-end space-x-3" ] [
            _a [
                _href_ $"/AccountsDetails?id={market.Id}"
                _class_
            ...
            _a [
                _href_ $"/AccountsEdit?id={market.Id}"
                _class_
