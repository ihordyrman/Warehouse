...
public class DashboardModel(IBalanceManager balanceManager, WarehouseDbContext db) : PageModel
{
    ...
    public async Task<IActionResult> OnGetAsync()
    {
        ...
        {
            Console.WriteLine($"ServiceError loading dashboard data: {ex.Message}");
        }
    ...
    private async Task LoadAccountsAsync()
    {
        ...
        foreach (MarketCredentials account in accounts)
        {
            ...
            Result<decimal> totalUsdt = await balanceManager.GetTotalUsdtValueAsync(account.Market.Type, CancellationToken.None);
