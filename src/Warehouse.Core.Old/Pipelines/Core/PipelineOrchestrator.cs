...
using System.Data;
using Dapper;
using Dapper.FSharp.PostgreSQL;
    ...
    IPipelineExecutorFactory executorFactory) : BackgroundService, IPipelineOrchestrator
{
    ...
    public async Task SynchronizePipelinesAsync()
    {
        ...
        var pipelines = (await db.QueryAsync<Pipeline>(
            "SELECT * FROM pipeline_configurations")).ToList();
        ...
        foreach (Pipeline pipeline in pipelines)
        {
            ...
            bool shouldBeRunning = pipeline.Enabled && pipeline.Status != PipelineStatus.Paused &&
    ...
    public async Task<bool> StartPipelineAsync(int pipelineId)
    {
        ...
        IDbConnection db = scope.ServiceProvider.GetRequiredService<IDbConnection>();
        var pipeline = await db.QueryFirstOrDefaultAsync<Pipeline>(
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (await periodicTimer.WaitForNextTickAsync(stoppingToken))
        {
            ...
            {
                logger.LogError(ex, "ServiceError during pipeline synchronization");
            }
        }
    }
    ...
    private async Task<bool> StartPipelineInternalAsync(Pipeline pipeline, IServiceProvider serviceProvider)
    {
        try
        {
            ...
            if (!validationResult.IsValid)
            {
                ...
                logger.LogWarning("Pipeline {PipelineId} ({PipelineName}) failed validation: {Errors}", pipeline.Id,
    ...
    private async Task<bool> StopPipelineInternalAsync(int pipelineId)
    {
        ...
        {
            logger.LogError(ex, "ServiceError stopping pipeline {PipelineId}", pipelineId);
            return false;
        }
    }
    ...
    {
        logger.LogInformation("PipelineOrchestrator stopping, shutting down {Count} running pipelines",
            runningExecutors.Count);
